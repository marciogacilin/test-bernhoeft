...
namespace Bernhoeft.GRT.ContractWeb.Domain.SqlServer.ContractStore.Interfaces.Repositories
{
    public interface IAvisoRepository : IRepository<AvisoEntity>
    {
        ...
        Task CriarAvisoAsync(AvisoEntity entity, CancellationToken cancellationToken = default);

        Task<AvisoEntity> ObterAvisoAsync(int id, TrackingBehavior tracking = TrackingBehavior.Default,
            CancellationToken cancellationToken = default);

        Task AlterarAvisoAsync(AvisoEntity entity, CancellationToken cancellationToken = default);
    }
}